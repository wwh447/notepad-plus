 ...
 * IDE运行时将自动加载同目录下所有文件.
 */
var zen_settings = Abbr;  //从 Abbr.json 加载扩展 JSON.

/**
 * 解析资源 (片段, 缩写, 变量, 等.) 来自 Zen Coding.
...
/**
 * 展开编辑器中当前行的zen缩写.
 * @param {zen_editor} 编辑器实例.
 * @param {String} [syntax] 语法类型 (html, css, etc.).
 * @param {String} [profile_name] 输出配置文件名称 (html, xml, xhtml).
 * @return {Boolean} 返回. <code>true</code> 如果缩写是扩展的.
 * 顺利地.
 */
function expandAbbreviation(editor, syntax, profile_name) {
	...
	var caret_pos = editor.getSelectionRange().end,  //得到的选择范围.
		abbr,
		content = '';
		
	if ( (abbr = findAbbreviation(editor)) ) {  //找到缩写更新到 abbr 变量.
		content = zen_coding.expandAbbreviation(abbr, syntax, profile_name);
		if (content) {
			editor.replaceContent(content, caret_pos - abbr.length, caret_pos);  //替换内容.
			return true;
		}
	}
...
function matchPair(editor, direction, syntax) {
	...
	var range = editor.getSelectionRange(),
		cursor = range.end,  //光标.
		range_start = range.start,  //开始范围.
		range_end = range.end,  //结束范围.
		//content = zen_coding.splitByLines(editor.getContent()).join('\n'),
		content = String(editor.getContent()),  //获取内容.
		range = null,  //范围.
		_r,
	...
	if (direction == 'in' && old_open_tag && range_start != range_end) {
		...
		//如果选取范围成功，并且结束位置之后有html注释 将其添加到选取范围中.
		if( !!range && content.substr(range[1],4) == "<!--" ){
			
			range[1] += 4;  //更新选取范围结束位置.
				
			while(true){  //将选取范围更新到注释结尾.
				if( content.substr(range[1],3) != "-->" ){
					range[1]++;
				}else{
					range[1] += 3;
					break;
...
function toggleComment(editor) {
	...
	return syntax == "css" ? toggleCSSComment(editor) : toggleHTMLComment(editor);  //根据文档类型选择注释方式.
...
function evaluateMathExpression(editor) {
	...
	if (r) {
		...
		// 代替积分分部: 11\2 => Math.round(11/2).
		expr = expr.replace(/([\d\.\-]+)\\([\d\.\-]+)/g, "$1/$2>>0");
		
		try {
			//var result = new Function('return ' + expr)();
			var result = eval(expr);  //计算表达式.
 ...
 * @param {zen_editor} editor.
 */
function colorConversion(editor) {
	
	var caretPos = editor.getCaretPos(),  //获取光标位置.
		text = editor.getSelection() || guessSelection();  //获取选择内容.
	
	var re_color_hex = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3}|[A-Fa-f0-9]{8})$/; //包含hex16进制颜色信息.
	var re_color_rgb = /^rgb(?:a|)\(/; //包含rgb或rgba颜色信息.
	var re_color_name = /^(aqua|black|blue|fuchsia|gray|green|lime|maroon|navy|olive|purple|red|silver|teal|white|yellow)$/; //包含name颜色信息(未实现).
	
	( text && (re_color_hex.test(text) || re_color_rgb.test(text) || re_color_name.test(text)) ) && editor.replaceSelectionContent( color(text) );
	
	/* 猜测选择内容. */
	function guessSelection(){
		var temp_char = editor.getContent().substr(caretPos,1),  //查找所需的临时字符.
			temp_index = caretPos,  //查找所需的临时位置.
			stop_char = "#:r";  //可以令搜索中断的关键字符.
		
		/* 向左边查找边界. */
		do {
			temp_char = editor.getContent().substr(temp_index -= 1, 1);
		} while ( stop_char.indexOf( temp_char ) == -1 && temp_char != "\t" && temp_char != "\n" && temp_index > 0);
		
		editor.createSelection(caretPos, temp_index);  //创建选项范围.
		
		return editor.getSelection();  //返回选中内容.
	}
	...
	function color(c){
		...
		var _temp = "";  //临时变量.
		switch(true){
			case re_color_hex.test(c):
					c = c.slice(1);  //匿去 #.
					(c.length == 3) && (c = c.replace(/(.)(.)(.)/,"$1$1$2$2$3$3"));  //3位转6位.
					(c.length == 8) && (function(){
						_temp = "," + (parseInt(c.slice(0,2),16)/256).toFixed(2).substr(1).replace(/0$/,"").replace(/^\.0$/,"0");  //16进制透明度转10进制.
						c = c.slice(-6);  //8位的伪装成6位.
					})();
					for(var k = 0,len = c.length; k < len; k+=2){
						_goalColor += parseInt(c.substr(k,2),16) + ",";
					}
					c = "rgb"+(_temp?"a":"")+"(" +_goalColor.slice(0,-1) + _temp + ")";
				break;
			case re_color_rgb.test(c):
					_goalColor = c.replace(re_color_rgb,"").slice(0,-1).split(",");  //分解成数组.
					(_goalColor.length > 3) ? (_goalColor[3] = (_goalColor[3] * 255>>0).toString(16)) : (_goalColor[3]="");  //透明度转16进制.
					c = "#" + _goalColor[3] + to16(_goalColor[0]) + to16(_goalColor[1]) + to16(_goalColor[2]);  //拼装信息.
				break;
			default:
	...
	/* 10进制转16进制加工. */
	function to16(txt){
		return (txt>>0).toString(16).replace(/^(.)$/,"$1$1");
	}
 ...
 * @param {zen_editor} editor.
 */
function cssFormatting(editor) {
	
	var context = editor.getSelection(),  //选择的内容.
		contextGoal = "",  //整理出的优雅代码.
		selectorNum = 0;  //选择器数量.
	
	if(!context.length) {
		alert("^_^请选择需要格式化的css.");
		return false;
	}
	
	try {
		selectorNum = context.match(/{|}/g).length;  //检查选择符.
		
		if( selectorNum % 2 ){  //选择器未完整选中.
			alert("选中的代码不太完整ho.");
			return false;
		}
		
		context = context.replace(/^\s+|\s+$/g,"").split(/}/);  //干掉开头和结尾的空隙并分组.
		
		for(var i = 0, len = context.length, p = null; i < len; i++){
			p = context[i].replace(/^\s+/,"").split(/{/);  //干掉开头间隙并分离出选择符和属性设置.
			p = { selector:p[0], attribute:p[1] };  //取个名字卖萌.
			
			p.selector = p.selector.replace(/\s+$/g,"").replace(/\, +|\,/g,",\n") + "{\n\t";  //干掉结尾的空隙并且整理分组选择符.
			p.attribute = tidyAttr(p.attribute) + "\n}\n";
			
			contextGoal += p.selector + p.attribute;  //将整理后的片段归纳.
		}
		
		contextGoal = contextGoal.replace(/\n+$/,"");  //干掉最后一个换行.
		
	} catch(exception) {  //不包含选择符.
		contextGoal = "\t" + tidyAttr(context).replace(/\t+/g,"");
	}
	...
	* @return {String} attr.
	*/
	function tidyAttr(attr){
		attr = attr.replace(/[\n\r\t]/g,"").replace(/; +/g,";").replace(/: /g,":").split(";");  //对元素整理并进行分组.
		attr.length -= 1;  //省略由于;产生的最后一个多余元素.
		return ( attr.sort().join(";").replace(/;/g,";\n\t") )+";";  //对属性进行排序返回结果.
	}
	
	editor.replaceSelection( contextGoal );
}
 ...
 * @param {zen_editor} editor.
 */
function xmlFormatting(editor) {
	var context = editor.getSelection();  //选择的内容.
	
	if(!context.length) {
		alert("^_^请选择需要格式化的xml.");
		return false;
	}
	
	//根据字母排序以后，更新选择内容.
	editor.replaceSelection( context.split("\n").sort().join("\n") );
}
...
/* ext. */
zen_coding.registerAction('color_conversion', colorConversion);
zen_coding.registerAction('css_formatting', cssFormatting);
zen_coding.registerAction('xml_formatting', xmlFormatting);
 ...
 */(function(){
	...
	/**
	 * 从 html 搜索匹配的标签.
	 * <code>start_ix</code> 位置.
	 * @param {String} 搜寻 html 代码.
	...
	function findPair(html, start_ix, mode, action) {
		...
		while (ix-- && ix >= 0) {
			...
			if (ch == '<') {  //如果当前字符为"<"则进行处理.
				...
				if ( (m = check_str.match(end_tag)) ) { //查找闭合标签.
					tmp_tag = tag(m, ix);
...
function reflectCSSValue(editor) {
	...
	var str = compoundUpdate(editor, doCSSReflection(editor));
...
/**
 * 更新图像大小: 读取图像 from image/CSS rule under caret.
 ...
 */(function(){
	...
	function addComments(node, i) {
		...
		if (id_attr || class_attr) {
			...
			comment_str += id_attr?id_attr:class_attr;
			node.end = node.end.replace(/>/, '>'+'<!-- end ' + comment_str + ' -->');
...
/**
 * 处理CSS的属性: 替代片段 css扩展增强.
 * @author 993.
 * @link .
 */
(function(){
	
	var re_important = /(.)\!$/;  // !important 添后缀的策略,包含感叹号.
	var re_number = /^[\d\s]+$/;  // number 替换的策略,只包含数字和空格.
	var re_number_count = /^[\+\-\*\/\d\s\.]+$/;  // 包含加减乘除和数字，不包含字母.
	...
	function process(tree, profile) {
		for (var i = 0, il = tree.children.length; i < il; i++) {
			...
			/* 值更新. */
			function dress(str,val){
				return str.replace(/(;?)$/,val);
			}
			
			var real = item.real_name;
			var content = item.content.toLowerCase();  //内容.
			var start = item.start.toLowerCase();  //开始.
			var end = item.end.toLowerCase();  //结束.
			var re = "";  //返回值.
...
var zen_editor = (function(){
	...
	function findNewlineBounds(text, from) {
		...
		/*
			2013-03-25 993 修改.
			修复读取最末行时 end 返回值少1的bug.
		*/
		return {start: start, end: end - text.length < -1 ? end : end + 1};
	}
	...
	return {
		...
		/**
		 * 替换编辑器内容或部分代码. (from <code>start</code> to 
		 ...
		 * @param {String} value 你用来替换的内容.
		 * @param {Number} [start] 开始内容的索引位置.
		 * @param {Number} [end] 结束内容的索引位置.
		 * @param {Boolean} [no_indent] 可能是确定是否缩进.
		 * @return {Object} editor.
		 */
		...
		/**
		 * 获取当前输出配置文件名称.(@see zen_coding#setupProfile)
		 * @return {String}
		 */
		...
		/**
		 * 将当前选择内容替换成指定内容不选中新内容.
		 * @param {String} val 你用来替换的内容.
		 * @return {Object} editor.
		 */
		replaceSelection: function(val) {
			...
			if (sel) {
				try {
					this.replaceContent(val, sel.start, sel.end);
				} catch(e) {}
			}
			return this;
		},
		...
		/**
		 * 将当前选择内容替换成指定内容并选中新内容.
		 * @param {String} val 你用来替换的内容.
		 * @return {Object} editor.
		 */
		replaceSelectionContent: function(val) {
			...
			if (sel) {
				try {
					this.replaceContent(val, sel.start, sel.end)
						.createSelection(sel.start, sel.start + val.length);
				} catch(e) {}
			}
			return this;
		}
...
addMenuItem("用缩写包围HTML.", 'wrap_with_abbreviation', 'Ctrl+Shift+A');
addMenuItem("选中整个标签.", 'match_pair_outward', 'Ctrl+Shift+D');
addMenuItem("下一个编辑点.", 'next_edit_point', 'Ctrl+Alt+]');
addMenuItem("上一个编辑点.", 'prev_edit_point', 'Ctrl+Alt+[');
addMenuItem("跳至配对标签.", 'matching_pair', 'Ctrl+Alt+L');
addMenuItem("并行操作code.", 'merge_lines', 'Ctrl+Alt+M');
addMenuItem("注释代码.", 'toggle_comment', 'Ctrl+Alt+/');
addMenuItem("标签闭合模式切换.", 'split_join_tag', 'Ctrl+\'');
addMenuItem("删除整个标签.", 'remove_tag', 'Ctrl+Shift+\'');

// v0.7.
addMenuItem("数学表达式test.", 'evaluate_math_expression', 'Ctrl+Y');

addMenuItem("选择下个DOM.", 'select_next_item', 'Ctrl+.');
addMenuItem("选择上个DOM.", 'select_previous_item', 'Ctrl+,');
addMenuItem('Reflect CSS Value', 'reflect_css_value', 'Ctrl+Shift+B');

// 993 ext.
addMenuItem("颜色转换.", 'color_conversion', 'Ctrl+Alt+C');
addMenuItem("格式化CSS.", 'css_formatting', 'Ctrl+Alt+F');
addMenuItem("格式化XML.", 'xml_formatting', 'Ctrl+Alt+Shift+F');